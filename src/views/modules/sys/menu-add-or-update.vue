<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="systemType" :label="$t('menu.systemType')" size="mini">
        <el-radio-group v-model="dataForm.systemType">
          <el-radio :label="0">{{ $t('menu.systemType0') }}</el-radio>
          <el-radio :label="1">{{ $t('menu.systemType1') }}</el-radio>
        </el-radio-group>
      </el-form-item>
      ...
      <el-form-item prop="remarks" :label="$t('menu.remarks')">
        <el-input v-model="dataForm.remarks" :placeholder="$t('menu.remarks')"></el-input>
      </el-form-item>
      <el-form-item prop="remarks" :label="$t('menu.appBackground')">
        <el-upload
            class="upload-demo"
            :action="uploadAction"
  ...
  methods: {
    handleSuccess(response, file, fileList){
      if(response.code == 200){
        this.dataForm.backgroundFileId = response.data[0].fileId;
    ...
    handleRemove(){
      this.dataForm.backgroundFileId = '';
    },
    init () {
      this.appImgList = [];
      this.visible = true
    ...
    getInfo () {
      this.$http.get(`/sys/menu/${this.dataForm.id}`).then(({ data: res }) => {
        ...
        //附件回显
        if(res.data.sysAttachmentEntity){
          this.appImgList = [{
