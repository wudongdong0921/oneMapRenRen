<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="status" label="头像" size="mini">
        <el-upload
...
export default {
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      ...
      if(res.code == 200){
        this.imageUrl = window.SITE_CONFIG['apiURL']+window.SITE_CONFIG['targetPath']['security'] + res.data[0].fileViewPath;
        this.dataForm.headUrl = res.data[0].fileViewPath;
      }
    },
    ...
    getInfo () {
      this.$http.get(`/sys/user/${this.dataForm.id}`).then(({ data: res }) => {
        ...
        if(this.dataForm.headUrl){
          this.imageUrl = window.SITE_CONFIG['apiURL']+window.SITE_CONFIG['targetPath']['security'] + this.dataForm.headUrl;
        }
        // 角色配置, 区分是否为默认角色
