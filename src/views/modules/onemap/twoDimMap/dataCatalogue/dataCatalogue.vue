...
<div class="datacataloguebox">
    ...
    <div class="tableBox">
        <el-table :data="tableData" @expand-change="tableChange" style="width: 100%; margin-bottom: 20px" row-key="sjmlxxbId" :expand-row-keys='defaultKeys' border :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
            ...
            <el-table-column prop="type" :label="$t('menu.type')" header-align="center" align="center">
                <template slot-scope="scope">
                    ...
                    <el-tag v-else size="small" type="info">分析</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="directoryOrder" :label="$t('menu.sort')" header-align="center" align="center"></el-table-column>
            <!-- <el-table-column prop="url" :label="$t('menu.url')" header-align="center" align="center" width="250" :show-overflow-tooltip="true"></el-table-column>
...
export default {
    ...
    methods: {
                ...
                if(!flag) {
                    for(let item of this.tableData) {
                        if(item.children.length) {
                            this.defaultKeys.push(item.sjmlxxbId)
                        }
                    }
                }
            }, (err) => {})
        ...
        tableChange(row,expanded) {
            let index = this.defaultKeys.indexOf(row.sjmlxxbId)
            if(expanded && index === -1) {
                this.defaultKeys.push(row.sjmlxxbId)
            }else if(index > -1){
        ...
        handleClick(data) {
            ...
            // Promise.all(this.$refs.addData.init()).then(() => {
            //     if (this.dataForm.id) {
            //         setTimeout(() => {
        ...
        deleteData(data) {
            this.$confirm("是否删除该条数据？", "提示", {
                ...
                if (data.children && data.children.length) {
                    ...
                    this.$apis.deleteDataDir(data.sjmlxxbId, (res) => {
                        this.$message({
                            message: '删除成功！',
                            type: 'success'
                        });
